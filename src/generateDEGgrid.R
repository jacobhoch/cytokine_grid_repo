...
getPercentDEGs <- function(cyto_A, cyto_B, filestem) {
  
  repol_DEGs_up <- readtxt(glue("./data/DE_results/{filestem}/{cyto_A}_{cyto_B}_vs_none_none_up.txt"))
  naive_DEGs_up <- readtxt(glue("./data/DE_results/{filestem}/none_{cyto_B}_vs_none_none_up.txt"))
  cytoB_DEGs_up <- readtxt(glue("./data/DE_results/{filestem}/{cyto_A}_none_vs_none_none_up.txt"))
  
  repol_DEGs_down <- readtxt(glue("./data/DE_results/{filestem}/{cyto_A}_{cyto_B}_vs_none_none_down.txt"))
  naive_DEGs_down <- readtxt(glue("./data/DE_results/{filestem}/none_{cyto_B}_vs_none_none_down.txt"))
  
  indep_DEGs <- c(naive_DEGs_up[naive_DEGs_up %in% repol_DEGs_up],
                  naive_DEGs_down[naive_DEGs_down %in% repol_DEGs_down])
  percent <- length(indep_DEGs) / length(c(naive_DEGs_up, naive_DEGs_down))
  
  percent <- length(naive_DEGs_up[(naive_DEGs_up %in% repol_DEGs_up) &
                                    !(naive_DEGs_up %in% cytoB_DEGs_up)]) /
    length(naive_DEGs_up[!(naive_DEGs_up %in% cytoB_DEGs_up)])
...
for (i in 1:length(conditions)){
  ...
  grid[cyto_A,cyto_B] <- getPercentDEGs(cyto_A, cyto_B, filestem)
...
ht <- Heatmap(as.matrix(grid), col=colors, rect_gp = gpar(col = "black", lwd=1.5),
              ...
              heatmap_legend_param = list(title="independence\nscore",
